...
const id=()=>randomUUID();
const defaults={title:'Comunidad Sanantes',subtitle:'El Podcast del Cáncer',intro:'Un espacio para aprender, escuchar y acompañarnos.',accent:'#d65337',donationGoal:'1000000',donationUrl:'',donationTitle:'Hagamos posible el próximo episodio',welcomePoints:'10',referralPoints:'20',privacyContact:'',privacyText:'',autoPublish:'1'};
...
async function body(req){if(req.body){if(typeof req.body==='string')return JSON.parse(req.body);return req.body;}let b='';for await(const c of req){b+=c;if(b.length>100000)fail('El contenido es demasiado grande',413)}return b?JSON.parse(b):{};}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(path.startsWith('admin')){
      ...
      if(path==='admin/post'&&method==='POST'){const title=text(b.title,250),slug=text(b.slug,100);if(!title||!/^[-a-z0-9]+$/.test(slug)||!text(b.body,50000)||!['draft','published'].includes(b.status))fail('Revisa título, slug, contenido y estado');const postImg=(typeof b.image==='string'&&b.image.startsWith('data:image/')&&b.image.length<=600000)?b.image:safeImage(b.image);await query('INSERT INTO posts(id,slug,title,excerpt,body,category,status,image) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET slug=excluded.slug,title=excluded.title,excerpt=excluded.excerpt,body=excluded.body,category=excluded.category,status=excluded.status,image=excluded.image,updated_at=CURRENT_TIMESTAMP',[b.id||id(),slug,title,text(b.excerpt,500),text(b.body,50000),text(b.category,80)||'Comunidad',b.status,postImg||'']);await audit(me.id,'Artículo guardado: '+title);return send(res,{ok:true});}
      ...
      if(path==='admin/donation'&&method==='POST'){const amount=Number(b.amount);if(!Number.isSafeInteger(amount)||amount<=0||amount>1000000000)fail('Introduce un aporte válido en COP');await query('INSERT INTO donations(id,amount,note) VALUES(?,?,?)',[id(),amount,text(b.note,300)]);await audit(me.id,'Aporte registrado: '+amount+' COP');return send(res,{ok:true});}

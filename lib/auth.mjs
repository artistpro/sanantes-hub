...
export function hashPassword(password,salt=randomBytes(16).toString('hex')){if(typeof password!=='string'||password.length<6)fail('La contraseña debe tener al menos 6 caracteres');const derivedKey=scryptSync(password,salt,64).toString('hex');return {hash:derivedKey,salt};}
export function verifyPassword(password,storedHash,storedSalt){if(!password||!storedHash||!storedSalt)return false;try{const derivedKey=scryptSync(password,storedSalt,64).toString('hex');return secretEqual(derivedKey,storedHash);}catch{return false;}}

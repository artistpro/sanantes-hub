...
export function origin(){const raw=process.env.APP_ORIGIN||(process.env.VERCEL_PROJECT_PRODUCTION_URL?'https://'+process.env.VERCEL_PROJECT_PRODUCTION_URL:process.env.VERCEL_URL?'https://'+process.env.VERCEL_URL:null);if(!raw)fail('Falta configurar APP_ORIGIN',503);const u=new URL(raw);if(process.env.VERCEL&&u.protocol!=='https:')fail('APP_ORIGIN debe usar HTTPS',503);return u.origin;}
